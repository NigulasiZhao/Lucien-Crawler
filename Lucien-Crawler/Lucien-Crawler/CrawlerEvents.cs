...
namespace Lucien_Crawler
{
    ...
    {
        public event EventHandler<CralwerOnStart> OnStart;//爬虫启动事件

        public event EventHandler<CrawlerOnComplete> OnCompleted;//爬虫完成事件

        public event EventHandler<CrawlerOnError> OnError;//爬虫出错事件
        ...
        public async Task<string> Start(Uri uri, string proxy = null)
        {
            return await Task.Run(() =>
            {
                ...
                try
                {
                    if (this.OnStart != null) this.OnStart(this, new CralwerOnStart(uri));
                    ...
                    if (this.OnCompleted != null) this.OnCompleted(this, new CrawlerOnComplete(uri, threadId, milliseconds, pageSource));
                }
                catch (Exception ex)
                {
                    if (this.OnError != null) this.OnError(this, new CrawlerOnError(uri, ex));
                }
                return pageSource;
            });
        }
